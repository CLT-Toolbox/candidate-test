<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <h2 class="text-xl font-semibold text-gray-800">{{ $layup->name }}</h2>
            </div>
            <div class="flex gap-2">
                <button x-data @click="$dispatch('open-modal', 'edit-layup')"
                ...
                <form method="POST" action="{{ route('suppliers.layups.destroy', [$supplier, $layup]) }}" class="inline" onsubmit="return confirm('Delete this layup and all its layers?')">
                    @csrf @method('DELETE')
                    <button type="submit" class="inline-flex items-center px-3 py-2 border border-red-300 text-red-600 text-sm rounded-lg hover:bg-red-50">
                        🗑️ Delete
                    </button>
                </form>
        ...
        <div class="grid grid-cols-2 gap-4 mb-6">
            <div class="bg-white rounded-xl shadow p-4">
                ...
                <div class="text-2xl font-bold text-green-700">
                    {{ number_format($layup->getTotalThicknessAttribute(), 0) }}mm
                </div>
            </div>
            <div class="bg-white rounded-xl shadow p-4">
                <div class="text-xs text-gray-400 uppercase mb-1">Total Layers</div>
                <div class="text-2xl font-bold text-green-700">{{ $layup->getPlyCountAttribute() }} Layers</div>
            </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-gray-700">{{ $layer->species_grade ?? 'N/A' }}</td>
                        <td class="px-6 py-4">
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="px-6 py-8 text-center text-gray-400">No layers found. Add one!</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <!-- Edit Layup Metadata Modal -->
    <x-modal name="edit-layup">
        <form method="POST" action="{{ route('suppliers.layups.update', [$supplier, $layup]) }}" class="p-6">
            @csrf
            @method('PATCH')
            <h2 class="text-lg font-semibold mb-4">Edit Layup Metadata</h2>

            <div class="mb-4">
                <x-input-label for="layup_code" value="Layup Code" />
                <x-text-input name="layup_code" class="mt-1 block w-full" value="{{ $layup->layup_code }}" required />
            ...
            <div class="mb-4">
                <x-input-label for="species_grade" value="Species/Grade" />
                <x-text-input name="species_grade" class="mt-1 block w-full" value="{{ $layup->species_grade }}" />
                <x-input-error :messages="$errors->get('species_grade')" class="mt-1" />
            </div>

            <div class="mb-4">
                <x-input-label for="revision" value="Revision" />
                <x-text-input name="revision" class="mt-1 block w-full" value="{{ $layup->revision }}" />
                <x-input-error :messages="$errors->get('revision')" class="mt-1" />
            </div>

            <div class="mb-4">
                <x-input-label for="status" value="Status" />
                <select name="status" class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm">
                    <option value="draft" {{ $layup->status === 'draft' ? 'selected' : '' }}>Draft</option>
    ...
    <x-modal name="create-layer">
        <form method="POST" action="{{ route('suppliers.layups.layers.store', [$supplier, $layup]) }}" class="p-6">
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                ...
                <div class="col-span-2">
                    <x-input-label for="species_grade" value="Species/Grade (e.g., C/24, C/16)" />
                    <x-text-input name="species_grade" class="mt-1 block w-full" placeholder="e.g. C/24" />
                    <x-input-error :messages="$errors->get('species_grade')" class="mt-1" />
                </div>
            </div>
            <div class="flex justify-end gap-2">
