        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-blue-50 dark:bg-blue-900/20 border-l-4 border-blue-500 p-4 mb-6 rounded-lg">
                <div class="flex items-start">
                    ...
                    <div>
                        <h3 class="text-sm font-semibold text-blue-800 dark:text-blue-300">Edit Supplier Information</h3>
                        <p class="text-sm text-blue-700 dark:text-blue-400 mt-1">
                            Update the supplier details below. Changes will be saved immediately.
                        </p>
                    </div>
                </div>
            </div>
                    ...
                    <form action="{{ route('dashboard.suppliers.update', $supplier) }}" method="POST">
                        ...
                        <div class="flex items-center gap-4 mb-6 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                            ...
                            <div class="ml-auto text-right">
                                ...
                                <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $supplier->created_at->format('M d, Y') }}</p>
                            </div>
                        </div>
                ...
                <div class="p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div class="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                            <p class="text-sm text-gray-500 dark:text-gray-400">Total Layups</p>
                            <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $supplier->layups->count() }}</p>
                        </div>
                        <div class="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                            <p class="text-sm text-gray-500 dark:text-gray-400">Total Layers</p>
                            <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $supplier->layups->sum(fn($l) => $l->layers->count()) }}</p>
                        </div>
                        <div class="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                            <p class="text-sm text-gray-500 dark:text-gray-400">Last Updated</p>
                            <p class="text-sm font-bold text-gray-900 dark:text-white">{{ $supplier->updated_at->format('M d, Y') }}</p>
                        </div>
                    </div>
                </div>
            </div>
