                    ...
                    <div class="flex justify-between items-center mt-4 mb-6">
                        <form action="{{ route('suppliers.index') }}" method="GET" class="flex items-center space-x-2">
                            <div class="relative">
                                ...
                                <x-text-input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama supplier..." class="pl-10 w-64 text-sm" />
                            </div>
                            ...
                            @if(request('search'))
                                <a href="{{ route('suppliers.index') }}" class="text-sm text-gray-500 hover:text-gray-700 underline">
                                    {{ __('Reset') }}
                                </a>
                            @endif
                        </form>
                                ...
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex justify-end items-center space-x-2">
                                            <x-primary-button href="{{ route('suppliers.layups.index', $supplier->id) }}">
                                                {{ __('Layups') }}
                                            </x-primary-button>
                                            <x-secondary-button x-data="" x-on:click.prevent="$dispatch('open-modal', 'edit-supplier-modal-{{ $supplier->id }}')" class="text-blue-600 dark:text-blue-500 hover:text-blue-900 dark:hover:text-blue-400" title="Edit">
                                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                                </svg>
                                                Edit
                                            </x-secondary-button>
                                            <x-secondary-button x-data="" x-on:click.prevent="$dispatch('open-modal', 'delete-supplier-modal-{{ $supplier->id }}')" class="text-red-600 dark:text-red-500 hover:text-red-900 dark:hover:text-red-400" title="Delete">
                                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
